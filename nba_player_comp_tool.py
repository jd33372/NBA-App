...
def load_and_process_data():
    ...
    try:
        ...
        st.error("NBA_career_stats.csv file not found. Please check the file path.")
        return None, None
...
if final_df is not None:
    ...
    def get_player_input():
        player_name = st.sidebar.selectbox("Player Name", final_df['player'].unique())
        num_comparisons = st.sidebar.selectbox("Number of Similar Players to Display", [1, 2, 3, 4, 5])
    ...
    if st.sidebar.button("Show Dataset Info"):
        ...
        st.write(f"**Total Players:** {len(final_df)}")
        st.write(f"**Available Columns:** {len(final_df.columns)}")
        ...
        st.write("### Top 10 Players by Career Score:")
        top_players = final_df.nlargest(10, 'Career Score')[['player', 'pos', 'Career Score']]
        st.dataframe(top_players, use_container_width=True)

else:
    st.error("Failed to load NBA data. Please check your data file and try again.")
